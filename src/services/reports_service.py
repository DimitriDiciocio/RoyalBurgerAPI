...
def get_reports(report_type, period):
    """Gera relatórios baseados no tipo e período solicitado."""
    conn = None
    try:
...
def _get_sales_report(cur, start_date, end_date):
    """Relatório de vendas por data e hora."""
    # Vendas por data
    cur.execute("""
    ...
    # Vendas por hora (média dos últimos 7 dias)
    cur.execute("""
...
def _get_performance_report(cur, start_date, end_date):
    ...
    # Tempo médio de preparo por dia
    cur.execute("""
        SELECT DATE(CREATED_AT) as date,
