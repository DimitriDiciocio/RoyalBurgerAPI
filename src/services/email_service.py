...
def _generate_fallback_html(user_name, subject, app_url=None):
    """
    Gera um HTML de fallback estilizado quando o template HTML falha.
    Segue o padrão visual dos outros emails do sistema.
    """
    order_id = None
    # Tenta extrair o número do pedido do subject
    if 'Pedido #' in subject:
        try:
            order_id = subject.split('Pedido #')[1].split(' ')[0]
        except:
            pass
    
    button_html = ''
    if order_id and app_url:
        button_html = f'        <div class="button-container"><a href="{app_url}/my-orders/{order_id}" class="button">Acompanhar Pedido</a></div>\n'
...
def send_email(to, subject, template, **kwargs):  
    ...
    try:
        ...
        try:
            ...
            # ALTERAÇÃO: Gera HTML de fallback estilizado seguindo o padrão do sistema
            user_name = kwargs.get('user', {}).get('full_name', 'Cliente') if isinstance(kwargs.get('user'), dict) else (kwargs.get('user') if kwargs.get('user') else 'Cliente')
            app_url = kwargs.get('app_url', None)
            msg.html = _generate_fallback_html(user_name, subject, app_url)
