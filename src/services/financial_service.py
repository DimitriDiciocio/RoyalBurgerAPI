...
def get_financial_summary(period='this_month'):
    """Retorna os KPIs financeiros para o período especificado."""
    conn = None
    try:
        ...
        if period == 'this_month':
            ...
            # Para outros períodos, usa os últimos 30 dias
            cur.execute("""
                SELECT 
                    SUM(CASE WHEN TYPE = 'revenue' THEN AMOUNT ELSE 0 END) as total_revenue,
                    SUM(CASE WHEN TYPE = 'expense' THEN AMOUNT ELSE 0 END) as total_expense
        ...
        # Saldo atual (receitas - despesas totais)
        cur.execute("""
            SELECT 
...
def get_financial_transactions(filters=None):
    """Lista todas as transações financeiras com filtros opcionais."""
    conn = None
    try:
        conn = get_db_connection()
        cur = conn.cursor()
