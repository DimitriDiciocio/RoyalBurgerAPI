...
# =============================
# Rotas para Fluxo On-Site (Mesa)
# =============================

@order_bp.route('/open-table', methods=['POST'])
@require_role('attendant', 'manager', 'admin')
def open_table_order_route():
    """
    Abre uma mesa para pedido on-site.
    Body: {"table_id": 5}
    """
    claims = get_jwt()
    attendant_id_raw = claims.get('sub')
    if not attendant_id_raw:
        return jsonify({"error": "Token inválido: user_id não encontrado"}), 401
    try:
    ...
    if not table_id:
        return jsonify({"error": "table_id é obrigatório"}), 400
    
    try:
        table_id = int(table_id)
    except (ValueError, TypeError):
        return jsonify({"error": "table_id deve ser um número inteiro"}), 400
    
    order_data, error_code, message = order_service.open_table_order(table_id, attendant_id)
    if order_data:
        return jsonify(order_data), 201
    
    if error_code == "TABLE_NOT_AVAILABLE":
        return jsonify({"error": message}), 409
    elif error_code == "TABLE_NOT_FOUND":
        return jsonify({"error": message}), 404
    elif error_code == "INVALID_ATTENDANT":
        return jsonify({"error": message}), 403
    elif error_code == "DATABASE_ERROR":
        return jsonify({"error": message}), 500
    else:
        return jsonify({"error": message or "Falha ao abrir mesa"}), 500


@order_bp.route('/<int:order_id>/items', methods=['POST'])
@require_role('attendant', 'manager', 'admin')
def add_items_to_table_order_route(order_id):
    """
    Adiciona itens a um pedido de mesa que está aberto.
    Body: {"items": [{"product_id": 1, "quantity": 2, "extras": [...]}]}
    """
    claims = get_jwt()
    attendant_id_raw = claims.get('sub')
    if not attendant_id_raw:
    ...
    order_data, error_code, message = order_service.add_items_to_table_order(order_id, items, attendant_id)
    if order_data:
    ...
    if error_code == "ORDER_NOT_FOUND":
    ...
    elif error_code == "INVALID_ORDER_TYPE":
        return jsonify({"error": message}), 400
    elif error_code == "EMPTY_ITEMS":
        return jsonify({"error": message}), 400
        ...
        return jsonify({"error": message or "Falha ao adicionar itens"}), 500


@order_bp.route('/<int:order_id>/close', methods=['POST'])
@require_role('attendant', 'manager', 'admin')
def close_table_order_route(order_id):
    """
    ...
    order_data, error_code, message = order_service.close_table_order(order_id, payment_method, attendant_id)
    if order_data:
        return jsonify({"msg": message, "order": order_data}), 200
    ...
    if error_code == "ORDER_NOT_FOUND":
        ...
        return jsonify({"error": message}), 400
    elif error_code == "EMPTY_ORDER":
        return jsonify({"error": message}), 400
    elif error_code == "MISSING_PAYMENT_METHOD":
        return jsonify({"error": message}), 400
        ...
        return jsonify({"error": message or "Falha ao fechar pedido"}), 500